...
export function SelectionInspector({
  ...
  return (
    <aside className="rounded-[26px] border border-white/10 bg-slate-950/88 p-4 shadow-[0_22px_55px_rgba(2,6,23,0.45)] backdrop-blur">
      <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-sky-200/78">Detalhes</p>
      <h4 className="mt-2 text-lg font-semibold text-white">{selectedItemId ? "Item selecionado" : "Resumo do mapa"}</h4>
      <p className="mt-2 text-sm leading-6 text-slate-300">
        {selectedItemId
          ? "Ajuste rotacao, escala e dados do item selecionado."
          : "Selecione um item da biblioteca e clique no mapa para posicionar."}
      </p>
      ...
      <div className="mt-4 grid gap-2">
        ...
        <MetricRow label="Itens colocados" value={placedItemsCount.toString()} />
        <MetricRow
          label="Cursor"
          value={hoveredCell ? `${hoveredCell.x}, ${hoveredCell.y}, ${hoveredCell.z}` : "Passe o cursor no mapa"}
        />
      </div>

      {selectedItem ? (
        <div className="mt-4 rounded-[22px] border border-amber-300/18 bg-amber-300/8 p-3">
          <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-amber-100">Edicao fina</p>
          <div className="mt-3 grid gap-2">
            <MetricRow label="ID do item" value={selectedItem.prefabId} />
          ...
          <div className="mt-3 grid gap-3">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">Rotacao rapida</p>
      ...
      <div className="mt-4 rounded-[22px] border border-white/10 bg-black/20 p-3">
        <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-400">Resumo da base</p>
