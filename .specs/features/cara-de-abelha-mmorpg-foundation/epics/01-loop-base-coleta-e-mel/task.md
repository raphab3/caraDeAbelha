...
- [x] `TASK-CDAM-01-03` Adicionar a acao `collect_flower` no protocolo e validar distancia, existencia da flor, capacidade da mochila e disponibilidade de polen.
- [x] `TASK-CDAM-01-04` Implementar a regra de deposito automatico ao entrar na area da colmeia, com conversao autoritativa de polen em mel.
- [x] `TASK-CDAM-01-05` Adicionar mensagens `player_status` e `interaction_result` sem misturar todo o HUD dentro de `state`.
- [ ] `TASK-CDAM-01-06` Estender `client/src/types/game.ts` e `client/src/hooks/useGameSession.ts` para armazenar progresso local e feedback efemero.
- [ ] `TASK-CDAM-01-07` Separar handlers de terreno e de entidades em `InstancedWorldField` para que flor seja clicavel sem disparar movimento de chao por engano.
